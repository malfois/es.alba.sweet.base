...
public class Json<T extends AFileConfiguration> {
	...
	public void read() {
		try {
			...
			Output.MESSAGE.info("es.alba.sweet.base.configuration.Json.read", "File " + this.file.toString() + " loaded");
		} catch (IOException e) {
			// e.printStackTrace();
			Output.MESSAGE.warning("es.alba.sweet.base.configuration.Json.read", "Error reading file " + this.file.toString() + ". Default settings loaded");
		}
	}
